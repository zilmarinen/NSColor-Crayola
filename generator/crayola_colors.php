...
function nscolor_from_hex($hex)
{
	...
	return "[NSColor colorWithRed:" . number_format($red_1, 2) . " green:" . number_format($green_1, 2) . " blue:" . number_format($blue_1, 2) . " alpha:1.0]";
}
...
function nscolor_name($name)
{
	return "crayola" . str_replace(" ", "", ucwords($name)) . "Color";
}

function nscolor_table_row($name, $hex, $width, $height, $image_url)
{
	$table_row = "\t<tr>\n";
	$table_row .= "\t\t<td><img src=\"$image_url" . image_filename($name) . "\" alt=\"" . nscolor_name($name) . "\" width=\"$width\" height=\"$height\" /></td>\n";
	$table_row .= "\t\t<td>$name</td>\n";
	$table_row .= "\t\t<td>[NSColor " . nscolor_name($name) . "]</td>\n";
	$table_row .= "\t\t<td>" . nscolor_from_hex($hex) . "</td>\n";
	$table_row .= "\t</tr>\n\n";
